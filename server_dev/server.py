...
class Server:
    ...
    def setup(self, setting):
        ...
        if setting:
            ...
            self._user_gml = GML.gml_import(constants.BBS04_CODE, bytes("./user-gml".encode())) # 무조건 파일로 밖에 못 불러옴. 파일명은 bytes() 써서 해줘야 됨
            ...
            self._store_gml = GML.gml_import(constants.BBS04_CODE, bytes("./store-gml".encode())) # 무조건 파일로 밖에 못 불러옴. 파일명은 bytes() 써서 해줘야 됨
            return
